...
class CDD:
    ...
    def __init__(self, max_k = 10,random = 0,method = 'gmm'):
        self.max_k = max_k
        self.random = random
        self.method = method
    ...
    def predict(self,new_data,update = 0.1,alpha = 0.05):
        ...
        def merge_clusters(cluster1,cluster2,fitted):
            ...
            new_fitted.append([merged_size,merged_mean,merged_cov])
            return new_fitted
        ...
        for m in tqdm(range(M)):
            ...
            if sum(diffrent_from) != concepts:
                ...
                anomalies[m] = 1
                if (count_obs_each_cluster[cluster] >= int(update*self.Clusters_data[cluster][2]))&(count_obs_each_cluster[cluster]>1):
