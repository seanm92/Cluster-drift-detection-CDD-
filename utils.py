...
class load_db:
    @staticmethod
    def load_intrusion(working_path):
        ...
        le = LabelEncoder()
        for i in [1,2,3]:
            KDD.ix[:,i] = le.fit_transform(KDD.ix[:,i])
            KDD_test.ix[:,i] = le.transform(KDD_test.ix[:,i])
        ...
        ###add zeros column in the missing category column
        #h1 = KDD_test[:,:67]
        #mid = np.zeros(KDD_test.shape[0])
        #h2 = KDD_test[:,67:]
        #KDD_test = np.concatenate((h1,mid[:,None],h2),axis = 1)
    ...
    def load_fraud(working_path):
        ...
        le = LabelEncoder()
        Fraud.ix[:,1] = le.fit_transform(Fraud.ix[:,1])
